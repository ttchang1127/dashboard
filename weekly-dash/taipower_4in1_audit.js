...
function renderLeavePanel() {
    ...
    return insightPanel({
        ...
        body: `
            ...
            ${h("罰則")}${fines}
            <p class="mt-4 rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-900">職安代理路線A（≤7天）與路線B（≤5天）<strong>可同月合併使用、分開計算</strong>（115/09/22 確認）；惟輔導要點十一(七)「每月累計逾7天應更換原職安人員」未區分路線，合計逾 7 天的月份宜在公文寫明兩路線各用幾天。待確認：觀音中大監造主任（10年／8年門檻）之合格代理人尚未安排。</p>
            <p class="mt-2 text-xs text-stone-400">依據：工作說明書 7.2、7.4.2、7.6.2～7.6.4、7.6.8、7.3.9、11.5.1.1.2、13.6、13.7；技術服務契約「工作安全與衛生」附件；承攬商安全衛生輔導要點十一、二十四及罰款標準（114.10.23）。確認事項：工地負責人＝監造主任、互為代理以出勤配置計畫預先核定、職安代理兩路線可同月合併（115/09/22）。</p>
        `
    });
}
